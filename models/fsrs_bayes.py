...
class FSRS6Bayes(FSRS6):
    ...
    init_w = torch.tensor([1.6576, 4.9108, 9.2206, 23.8137, 8.8253, 0.3109, 3.878, 0.1196, 1.1004, 0.2169, 1.4507, 0.6544, 0.1271, 0.4549, 2.4284, 0.2679, 3.3245, 1.3595, 0.515, 0.2089, 0.2137, 
                           1.6926, 4.9188, 9.2263, 23.9065, 5.0538, 0.2919, 3.8809, 0.1204, 2.2128, 0.1324, 1.2495, 1.6205, 0.1169, 0.419, 3.6272, 0.1886, 5.2914, 0.627, 0.1111, 0.1339, 0.3818], dtype=torch.float32)
    ...
    def init1(self, train_set) -> None:
        ...
        for first_rating in ("1", "2", "3", "4"):
            ...
            def loss(stability):
                y_pred = self.forgetting_curve(delta_t, stability, -self.ws[20].item())
                logloss = sum(
    ...
    def init2(self, train_set) -> None:
        ...
        for first_rating in ("1", "2", "3", "4"):
            ...
            def loss(stability):
                y_pred = self.forgetting_curve(delta_t, stability, -self.ws[41].item())
                logloss = sum(
                    -(recall * np.log(y_pred) + (1 - recall) * np.log(1 - y_pred))
                    * count
                )
