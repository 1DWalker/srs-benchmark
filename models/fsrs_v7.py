...
class FSRS7ParameterClipper(FSRS6ParameterClipper):
    def __call__(self, module):
        if hasattr(module, "w"):
            ...
            w[26] = w[26].clamp(0, 1)
            # Forgetting curve
            w[27] = w[27].clamp(0.01, 0.99)  # decay 1
            w[28] = w[28].clamp(0.01, 0.99)  # decay 2
            w[29] = w[29].clamp(0.01, 0.99)  # decay 3
            w[30] = w[30].clamp(0.5, 0.99)  # base 1
            w[31] = w[31].clamp(0.5, 0.99)  # base 2
            w[32] = w[32].clamp(0.5, 0.99)  # base 3
...
class FSRS7(FSRS6):
    ...
    def pretrain(self, train_set: pd.DataFrame) -> None:
        ...
        def evaluate_param_set(param_set):
            ...
            decay1, decay2, decay3, base1, base2, base3, weight1, weight2, weight3 = param_set
        ...
        if best_forgetting_curve_params is not None:
            # forgetting_curve
            self.w.data[-9:] = Tensor(best_forgetting_curve_params)
    ...
    def get_optimizer(self, lr, wd, betas):
        return torch.optim.NAdam(params=self.parameters(), lr=lr, weight_decay=wd, betas=betas)