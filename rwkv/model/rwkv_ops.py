...
class RWKV7_Packed_WKV(torch.autograd.Function):
    @staticmethod
    def forward(ctx, *inputs):
        indices_I, r_THK, k_THK, v_THK, w_THK, a_THK, k_deformed_THK = inputs
        assert all(
            i.is_contiguous()
            for i in [indices_I, r_THK, k_THK, v_THK, w_THK, a_THK, k_deformed_THK]
        )
        # assert all(not torch.isnan(i).any() for i in [r_BTHK, k_BTHK, v_BTHK, w_BTHK, a_BTHK, k_deformed_BTHK, skip_BT])
        assert w_THK.dtype == torch.float32
        assert indices_I.dtype == torch.long
        dtype = r_THK.dtype
        assert all(
            i.dtype == dtype for i in [r_THK, k_THK, v_THK, a_THK, k_deformed_THK]
        )
        if r_THK.dtype == torch.bfloat16:
            out, state_checkpoints = (
                torch.ops.rwkv.rwkv7_packed_wkv_forward_bfloat16.default(
                    indices_I, r_THK, k_THK, v_THK, w_THK, a_THK, k_deformed_THK
                )
            )
        elif r_THK.dtype == torch.float:
            out, state_checkpoints = torch.ops.rwkv.rwkv7_packed_wkv_forward_float.default(
                indices_I, r_THK, k_THK, v_THK, w_THK, a_THK, k_deformed_THK
            )
        elif r_THK.dtype == torch.half:
            out, state_checkpoints = torch.ops.rwkv.rwkv7_packed_wkv_forward_half.default(
                indices_I, r_THK, k_THK, v_THK, w_THK, a_THK, k_deformed_THK
            )
    ...
    def backward(ctx, grad_THK):
        ...
        assert r_THK.is_cuda
        if r_THK.dtype == torch.bfloat16:
            r_grad, k_grad, v_grad, w_grad, a_grad, k_deformed_grad = (
                torch.ops.rwkv.rwkv7_packed_wkv_backward_bfloat16.default(
                    indices_I,
                    r_THK,
                    k_THK,
                    v_THK,
            ...
            r_grad, k_grad, v_grad, w_grad, a_grad, k_deformed_grad = (
                torch.ops.rwkv.rwkv7_packed_wkv_backward_float.default(
                    indices_I,
                    r_THK,
                    k_THK,
                    v_THK,
            ...
            r_grad, k_grad, v_grad, w_grad, a_grad, k_deformed_grad = (
                torch.ops.rwkv.rwkv7_packed_wkv_backward_half.default(
                    indices_I,
                    r_THK,
                    k_THK,
                    v_THK,
        ...
        return None, r_grad, k_grad, v_grad, w_grad, a_grad, k_deformed_grad


def reference_rwkv7_packed(indices_I, r_THK, k_THK, v_THK, w_THK, a_THK, k_deformed_THK):
    T = r_THK.size(0)
    sizes_I = torch.cat((indices_I, torch.tensor([indices_I.size(0)], device=indices_I.device)))
    sizes_I = (sizes_I[1:] - sizes_I[:1]).tolist()
    results = []
    splits = indices_I.tolist()
    for i in range(len(splits)):
        begin_i = splits[i]
        end_i = T if i == len(splits) - 1 else splits[i + 1]
        r_BSHK = r_THK[begin_i:end_i].unsqueeze(0)
        k_BSHK = k_THK[begin_i:end_i].unsqueeze(0)
        v_BSHK = v_THK[begin_i:end_i].unsqueeze(0)
        w_BSHK = w_THK[begin_i:end_i].unsqueeze(0)
        a_BSHK = a_THK[begin_i:end_i].unsqueeze(0)
        k_deformed_BTHK = k_deformed_THK[begin_i:end_i].unsqueeze(0)
        B, S, _, _ = r_BSHK.shape
        skip_BT = torch.full((B, S), 0, dtype=torch.bool, device=r_BSHK.device)
        out_BTHK = reference_rwkv7(r_BSHK, k_BSHK, v_BSHK, w_BSHK, a_BSHK, k_deformed_BTHK, skip_BT)
        results.append(out_BTHK.squeeze(0))
