...
def train(model, inner_opt_state, train_df_list, test_df_list):
    ...
    for outer_it in range(1, OUTER_STEPS + 1):
        ...
        if len(pool) == 0:
            pool = list(range(len(train_df_list)))
            random.shuffle(pool)

        task_id = pool.pop()
        # print(task_id, len(pool))
        ...
        learner = copy.deepcopy(model)
        # fsrs init
        learner.initialize_parameters(train_df_list[task_id])
        inner_opt = get_inner_opt(learner.parameters())
        ...
        if len(wandb_log) > 0 and use_wandb:
...
def main():
    from models import FSRS6Bayes, LSTM
    print("Reminder: set init_param of fsrs to use the current w, not the initial w")

    if MODEL_NAME == "FSRS-6-bayes":
        model = FSRS6Bayes(config)
    ...
    results = list(map(process_user, all_users))
