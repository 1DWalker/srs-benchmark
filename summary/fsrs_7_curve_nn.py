...
def fsrs7_step(l: int, p: FSRS7Buffer, feature_elapsed, feature_rating, stability, difficulty, curve_nn):
    if l == 0:
        new_s = p.s.gather(dim=0, index=(feature_rating - 1).clamp_min(0))
        new_d = init_d(p.init_d0, p.init_d1, feature_rating).clamp(1, 10)
    else:
        r = curve_nn(feature_elapsed, stability, difficulty)
...
def forward(parameters_p, feature_elapsed_days_real_bl, feature_rating_bl, label_elapsed_days_real_bl, curve_nn):
    ...
    for feature_elapsed, feature_rating in zip(feature_elapsed_days_real_bl.transpose(0, 1), feature_rating_bl.long().transpose(0, 1)):
        stability, difficulty = fsrs7_step(l, p, feature_elapsed, feature_rating, stability, difficulty, curve_nn)
        stabilities.append(stability)
        difficulties.append(difficulty)
        l += 1

    stability_tensor = torch.stack(stabilities, dim=-1)
    difficulty_tensor = torch.stack(difficulties, dim=-1)

    return curve_nn(label_elapsed_days_real_bl, stability_tensor, difficulty_tensor), stability_tensor
