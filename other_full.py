...
from utils import catch_exceptions, rmse_matrix, save_tensor
...
class FSRS6(FSRS):
    ...
    def iter(
        ...
        output = {
            ...
            "stabilities": stabilities,
            "difficulties": difficulties,
            "decays": torch.full_like(retentions, self.w[20].item())
        }
        output["penalty"] = (
            torch.sum(
                torch.square(self.w - self.init_w_tensor)
    ...
    def batch_predict(self, dataset):
        ...
        with torch.no_grad():
            for batch in batch_loader:
                ...
                if "decays" in result:
                    decays.extend(result["decays"].cpu().tolist())

        return retentions, stabilities, difficulties, decays
...
def create_features_full(df, model_name=MODEL_NAME, secs_ivl=SECS_IVL):
    ...
    if model_name.startswith("FSRS") or model_name in (
        ...
        df["tensor"] = [
            torch.tensor((t_item, r_item), dtype=torch.float32).transpose(
                0, 1
            )
...
def process(user_id):
    ...
    dataset_train = create_features(df_revlogs.copy(), MODEL_NAME, SECS_IVL)
    dataset_full = create_features_full(df_revlogs.copy(), MODEL_NAME, SECS_IVL)
    assert len(df_revlogs) == len(dataset_full), "rows must remain untouched"
    if dataset_train.shape[0] < 6:
    ...
    if PARTITIONS != "none":
        ...
        dataset_train = dataset_train.merge(df_cards, on="card_id", how="left").merge(
            df_decks, on="deck_id", how="left"
        )
        dataset_train.fillna(-1, inplace=True)
        if PARTITIONS == "preset":
            dataset_train["partition"] = dataset_train["preset_id"].astype(int)
        elif PARTITIONS == "deck":
            dataset_train["partition"] = dataset_train["deck_id"].astype(int)
    else:
        dataset_train["partition"] = 0
    w_list = []
    testsets = []
    tscv = TimeSeriesSplit(n_splits=n_splits)
    bounds = []
    for split_i, (train_index, _) in enumerate(tscv.split(dataset_train)):
        train_set = dataset_train.copy()
        train_set = dataset_train.iloc[train_index]
        bounds.append(int(train_set["review_th"].max()) + 1)

    bounds.append(int(1e9))

    for split_i, (train_index, _) in enumerate(tscv.split(dataset_train)):
        train_set = dataset_train.iloc[train_index]
        testsets.append(dataset_full[(bounds[split_i] <= dataset_full["review_th"]) & (dataset_full["review_th"] < bounds[split_i + 1])].copy())
    ...
    s = np.array([-1.0 for _ in range(len(dataset_full))])
    decay_list = np.array([-1.0 for _ in range(len(dataset_full))])
