...
namespace fsrs {
    TORCH_LIBRARY(fsrs, m) {
        ...
        m.def("fsrs_optimizer(Tensor pretrain_params, Tensor epochs, Tensor locs, Tensor locs_lens, Tensor keys, Tensor keys_lens, Tensor train_set_locs, Tensor train_set_keys, Tensor test_set_locs, Tensor test_set_keys, Tensor packed_review_th_T, Tensor packed_rating_T, Tensor packed_elapsed_days_real_T, Tensor packed_elapsed_days_int_T, Tensor packed_label_elapsed_days_real_T, Tensor packed_label_elapsed_days_int_T) -> (Tensor, Tensor, Tensor)");
    }
}